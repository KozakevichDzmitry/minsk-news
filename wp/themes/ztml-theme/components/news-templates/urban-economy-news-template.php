...
function render_urban_economy_news_template($is_minimazed = false, $is_district_page = false, $tax_slug = '', $type_adv = "page", $id = false)
{
	...
	render_topic_bar("Городское хозяйство", true, array(
		'link' => get_site_url() . '/news-list/cityhold/',
	...
	<div class="box-column-gap urban-economy-news cityhold" style="margin-bottom: 30px;">
		<?php if ($is_minimazed) : ?>
			<?php $urban_economy_attached_posts = get_attached_news('3', 'cityhold', $tax_query); ?>
			<div class="box-line-reverse-gap">
				<div class="box" data-postid="<?php echo $urban_economy_attached_posts['posts'][0]->ID;?>">
					<?php render_new_template_image($urban_economy_attached_posts['posts'][0]->ID); ?>
				</div>
				<div class="box box-list">
					<?php render_news_template_line($urban_economy_attached_posts['posts'][1]->ID, false, true); ?>
					<?php render_news_template_line($urban_economy_attached_posts['posts'][2]->ID, false, true); ?>
				</div>
			</div>
		<?php else : ?>
			<?php $urban_economy_attached_posts = get_attached_news('3', 'cityhold', $tax_query, true); ?>
			<?php $urban_economy_posts = get_default_news('4', 'cityhold', $urban_economy_attached_posts['ids']); ?>
			<div class="box">
			...
			<div class="box-line-gap">
				<div class="box" data-postid="<?php echo $urban_economy_attached_posts['posts'][2]->ID;?>">
