...
function share_links($post_ID=null)
{
    ...
    $share_block .= '<div onclick="share_popup(\'https://telegram.me/share/url?url=' . $modern_link . '&text=' . get_the_title($post->ID) . '\', \'Поделиться в Телеграме\', 580, 415)" class="nk_share_button nk_share_button--telegram">';
    $share_block .= render_telegram_share_icon() . '</div>';
    $share_block .= '<div onclick="share_popup(\'https://vk.com/share.php?url=' . $modern_link . '&title=' . get_the_title($post->ID) . '&image=' . get_the_post_thumbnail_url($post) . '\', \'Поделиться ВКонтакте \', 650, 600)" class="nk_share_button nk_share_button--vk">'; $share_block .= render_vk_share_icon() . '</div>';
    $share_block .= '<div onclick="share_popup(\'https://twitter.com/intent/tweet?text=' . get_permalink($post->ID) . ' ' . urlencode($post->post_title) . '\', \'Твитнуть\', 580, 415)" class="nk_share_button nk_share_button--tw">';
